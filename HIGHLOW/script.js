...
function ifButtonClickIsUp() {
	...
	if (isNaN(parseInt(card2Num)) && card2Num !== 'JOKER' && isNaN(parseInt(card1Num)) && card1Num !== 'JOKER') {
		if (
			(card1Num === 'J' && card2Num === 'Q') ||
			(card1Num === 'Q' && card2Num === 'K') ||
			(card1Num === 'K' && card2Num === 'A') ||
			(card1Num === 'Q' && card2Num === 'A')
		) {
			money = money + parseInt(betMoney.value * highMoneyPer)
			h2.innerText = `${money}`
			$().ready(function () {
...
function calculatorMoney() {
	...
	let downCount = 0,
		upCount = 0
	for (let i = 1; i < parseInt(card1Num); i++) {
		//  LOW 계산
		downCount++
	}
	for (let i = parseInt(card1Num); i < 15; i++) {
		// HIGH 계산
		upCount++
	}
